...
class BisimAgent(object):
    ...
    def update_encoder(self, obs, action, next_obs, curr_reward, L, step):
        ...
        loss = (z_dist - r_dist - self.discount *  transition_dist).pow(2).mean() + self.decoder_latent_lambda * latent_loss
    ...
    def update_encoder_onpolicy(self, obs, L, step):
        ...
        with torch.no_grad():
            ...
            if next_latent_sigma is None:
                next_latent_sigma = torch.ones_like(next_latent_mu)
    ...
    def update(self, replay_buffer, L, step):
        ...
        encoder_loss = self.update_encoder(obs, action, next_obs, reward, L, step)
        total_loss = 0.01 * encoder_loss + transition_reward_loss
        self.encoder_optimizer.zero_grad()
